...
export class CustomFormulaComponent implements OnInit {
  ...
  onFormulaInput(event: any) {
    ...
    if (this.currentWord.length > 0) {
      this.filteredSuggestions = this.suggestionsMetrics.filter((metric:any) =>
  ...
  validateFormula(formula: string): boolean {
    ...
    const availableMetrics = this.suggestionsMetrics.map((m:any) => m.metric.toLowerCase());
    const unavailableMetrics: string[] = [];

    matches.forEach(match => {
      const matchLower = match.toLowerCase();
      // Skip common mathematical functions and operators
      const mathFunctions = ['abs', 'sqrt', 'pow', 'min', 'max', 'floor', 'ceil', 'round'];
      if (!mathFunctions.includes(matchLower) && !availableMetrics.includes(matchLower)) {
  ...
  calculateFormulaValue(formula: string): number | null {
    try {
      ...
      this.suggestionsMetrics.forEach((metric:any) => {
        ...
        processedFormula = processedFormula.replace(regex, metric.value.toString());
      });
  ...
  addNewFormula() {
    ...
    if (this.editingFormulaId !== null) {
      ...
      const index = this.customFormulaList.findIndex(f => f.id === this.editingFormulaId);
      if (index !== -1) {
